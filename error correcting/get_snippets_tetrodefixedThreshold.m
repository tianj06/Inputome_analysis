function get_snippets_tetrodeNew(TT_id,wire_id,crosstype,ref,extName,thresh)
...
figure;plot(channel_data(1:100000))
...
for j=1:num_steps
    ...
    if strcmp(crosstype(1),'p')
        ind = crossing(this_window-thresh); 
        crossings = [crossings this_window_t(1) + ind];
    elseif strcmp(crosstype(1),'v')
        ind = crossing(this_window+thresh); 
        crossings = [crossings this_window_t(1) + ind];
    else
        ind = crossing(abs(this_window)-thresh); 
        crossings = [crossings this_window_t(1) + ind];        
    end
