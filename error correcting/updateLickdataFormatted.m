...
function saveFile = updateLickdataFormatted(dataPath,spike_file,savePath)
...
[a,b] = butter(5,.5,'low');
filtlick = filtfilt(a,b,difflick);
filtdiff = filtlick;%[0; diff(filtlick)];
...
for m = 1:length(spike_file)
    ...
    load(saveFile,'responses')
    responses.lickTimes = lickTimes;
    save(saveFile, '-append','responses')
end
