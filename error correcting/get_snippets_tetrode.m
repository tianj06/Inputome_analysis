function get_snippets_tetrode(filename,file_id,crosstype,ref)
% example get_snippets_tetrode('CSC11.dat','3','v',2)
...
Nsamples = length(channel_data);
channel_data = zeros(4,Nsamples);
for i = 1:4
     tempfile = ['CSC' num2str(i+4*floor((wireNum-1)/4)) extName];
     dirInfo = dir(tempfile);
     if dirInfo.bytes > 10000000
        [a, ~, ~] = readCSCfile(tempfile);
        if length(a)> Nsamples % sometimes files have slight different sizes
            channel_data(i,:)=a(1:Nsamples); 
        else
            channel_data(i,1:Nsamples)=a;
        end
     end
end
