...
validT = ~temp;
averagePSTH = squeeze(nanmean(rawPSTH(validT,:,:)));
averagelightPSTH = squeeze(nanmean(rawPSTH(lightIdx&validT',:,:)));
...
figure('Position',[360 50 1000 920]);
subplot(4,1,1)
for i = [1 2 4 5]
        smoothed = smooth(averagePSTH(trialType(i),:),100);    
        plot(smoothed,'color',CueColor(i,:),'LineWidth',1.5)
        i = i+1;
        hold on
end
...
%prettyP([200 4800],'','','','a')
xlim([200 4800])
title(sprintf('all neurons US+ %d out of %d %.1f%%', sum(validT),...
    length(validT),100*sum(validT)/length(validT)))

subplot(4,1,2)
for i = [1 2 4 5]
        smoothed = smooth(averagelightPSTH(trialType(i),:),100);    
        plot(smoothed,'color',CueColor(i,:),'LineWidth',1.5)
        i = i+1;
        hold on
end
...
%prettyP([200 4800],'','','','a')
xlim([200 4800])
title(sprintf('input neurons US+ %d out of %d %.1f%%', sum(validT&lightIdx'),...
    sum(lightIdx),100*sum(validT&lightIdx')/sum(lightIdx)))

validT = ~isnan( squeeze(rawPSTH(:,5,1)));
averagePSTH = squeeze(nanmean(rawPSTH(validT,:,:)));
averagelightPSTH = squeeze(nanmean(rawPSTH(lightIdx&validT',:,:)));
subplot(4,1,3)
for i = 1:3
        smoothed = smooth(averagePSTH(i+4,:),200);    
        plot(smoothed,'color',CueColor(i,:),'LineWidth',1.5)
        i = i+1;
        hold on
end
...
subplot(4,1,4)
for i = 1:3
        smoothed = smooth(averagelightPSTH(i+4,:),200);    
        plot(smoothed,'color',CueColor(i,:),'LineWidth',1.5)
        i = i+1;
        hold on
end
...
%%
%
%nanIdx = find(isnan(dataToCluster)); % debug the missing data
%[I, J] = ind2sub(size(dataToCluster),nanIdx);
for i = 1:10
    nclusters =i;
    minD = inf;
    for k = 1:50
